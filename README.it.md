...
Un gioco di ruolo basato su simulazioni in cui Claude orchestra la storia, il motore preserva la verità e i mondi evolvono attraverso voci, pressioni, fazioni, relazioni, economia e sistemi narrativi, portando a conclusioni significative. Giocalo o sviluppalo ulteriormente.
...
| Vuoi... | Usare |
|------------|-----|
| **Play right now** | `npx claude-rpg play` (selezione interattiva del mondo e del personaggio) |
| **Create a new world** | `npx claude-rpg new "your world concept"` |
| **Author worlds visually** | [World Forge](https://github.com/mcp-tool-shop-org/world-forge): studio di creazione 2D con editor di mappe, creatore di PNG e strumento di validazione. |
| **Validate world data** | [Cannon Archive](https://github.com/mcp-tool-shop-org/cannon-archive): validazione dello schema, test della storyboard, pipeline di esportazione. |
...
La versione 1.6 rende reali i dieci mondi, conferisce un peso reale alla sconfitta e rende il narratore resiliente anche quando la rete non è disponibile.
...
| **Flag `--world`** | `npx claude-rpg play --world gladiator` salta il menu e ti porta direttamente a un mondo specifico. Dieci alias, tutti elencati in `--help`. |
| **Death is a setback** | La sconfitta in combattimento si traduce in una schermata di morte distinta e limita le tue azioni fino alla resurrezione: le campagne terminano intenzionalmente tramite `/conclude`, non a causa di un singolo combattimento andato male. |
| **Streaming narration** | Il testo viene visualizzato mentre il narratore lo scrive, non dopo. |
| **`/cost` su richiesta** | Utilizzo del token di sessione e spesa stimata, senza dover spendere nulla per chiedere. |
| **Un indicatore che dice la verità** | Durante i tentativi di riconnessione all'API, l'indicatore animato mostra il tentativo e la causa: "ancora in attesa (tentativo 1/2 - limite di velocità raggiunto)". In caso di interruzioni prolungate, il testo di fallback passa a un onesto "questo sta ancora accadendo". |
| **Ambient world chatter** | I PNG sullo sfondo svolgono le loro attività: un mercante che controlla i prezzi, una guardia che sorveglia la folla, tutto personalizzato per ogni mondo, senza costi aggiuntivi per l'API. |
...
| **Names, not slugs** | La barra di stato, i riepiloghi e gli elenchi dei salvataggi mostrano "Cavaliere penitente", non `penitent-knight`. Gli indizi sonori vengono letti come parole, non come `white_noise`. |
| **1.542 test** | Aumentati da 625 su 95 file, con soglie di copertura per ogni percorso applicate nel sistema CI. |

## Perché è diverso?

| Cosa | Come |
|------|-----|
| **Verità della simulazione separata dalla narrazione** | Il motore risolve il combattimento, i movimenti, i dialoghi: Claude narra solo il risultato. Nessun esito frutto di allucinazioni. |
...
| **Ambiente immersivo audio/vocale in tempo reale** | I piani di narrazione strutturati guidano la sintesi vocale, gli effetti sonori, i livelli ambientali e la musica attraverso voice-soundboard. |
...
| **Autonomia dei PNG con catene di conseguenze** | I PNG agiscono in base agli obiettivi, tengono traccia degli obblighi e reagiscono quando i punti di svolta della lealtà cambiano. `/npc` e `/people` evidenziano i punti di svolta, gli angoli di influenza e le catene di conseguenze attive. |
...
| **Influenza del giocatore e azioni politiche** | Si può spendere influenza, favori e informazioni in azioni sociali, di diffusione di voci, diplomatiche o di sabotaggio. `/leverage` mostra il proprio capitale politico. |
| **Provenienza degli oggetti e reliquie** | Gli oggetti hanno una storia. Una spada che uccide abbastanza nemici diventa una reliquia con un epitaffio. Gli NPC riconoscono gli oggetti equipaggiati e reagiscono. `/item` analizza la provenienza e registra le informazioni. |
...
| Modulo | Scopo |
|--------|---------|
| **Macchina a stati di presentazione** | Tiene traccia dell'esplorazione, del dialogo, del combattimento e delle conseguenze, guidando la selezione dei livelli audio. |
...
| **Audio Director** | Pianifica le indicazioni con priorità, attenuazione del volume, tempi di attesa e anti-spam. |
| **Sound Registry** | Voci audio accessibili tramite indirizzo di contenuto: si possono interrogare per tag, umore e intensità. |
...
| Progetto | Cosa fa |
|---------|-------------|
| [AI RPG Engine](https://github.com/mcp-tool-shop-org/ai-rpg-engine) | Runtime di simulazione deterministico: 29 moduli, zero dipendenze LLM. |
...
| [Cannon Archive](https://github.com/mcp-tool-shop-org/cannon-archive) | Validazione dello schema, test della storyboard, pipeline di esportazione RPG AI. |
