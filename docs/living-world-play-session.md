...
| 9 | `/rumors` | Under **WHAT THE STREET BELIEVES**, the rumor about you with its witness count and how far it has mutated. Walk back up (`go to the vestry passage`, `go to the nave`) and `talk to brother aldric`; then `go to the chapel entrance` and `talk to the pilgrim`. Each named NPC hears the rumor the round you share their district; each one's dialogue prompt then carries it with THEIR stance (believe if their faction already took it up, doubt otherwise). Sister Maren is not a named NPC and never hears it — the street talks through its named people. |
